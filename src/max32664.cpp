...
uint8_t max32664::readRawSamples(uint32_t *irBuff, uint32_t *redBuff)
{
  ...
  uint8_t readLen = 12;
  uint8_t readBuff[MAX_BUFF_LEN] = {0};
  ...
  readMultipleBytes(0x12, 0x01, readBuff, (readLen * numSamples));

  for (size_t i = 0; i < numSamples; i++)
  {
    uint32_t ppgFinal = (uint32_t)readBuff[(readLen * i) + 0] << 16;
    ppgFinal |= (uint32_t)readBuff[(readLen * i) + 1] << 8;
    ppgFinal |= (uint32_t)readBuff[(readLen * i) + 2];

    uint32_t redFinal = (uint32_t)readBuff[(readLen * i) + 3] << 16;
    redFinal |= (uint32_t)readBuff[(readLen * i) + 4] << 8;
    redFinal |= (uint32_t)readBuff[(readLen * i) + 5];
...
bool max32664::readMultipleBytes(uint8_t data1, uint8_t data2, uint8_t *readBuff, uint16_t readLen)
{
  ...
  if (readLen > MAX_BUFF_LEN)
    readLen = MAX_BUFF_LEN;

  Wire.requestFrom(SENSORHUB_ADDR, readLen);
  statusByte = Wire.read();
...
bool max32664::startBPTcalibration()
{
  ...
  while (bpStatus != 2 && (max32664Output.progress != 100))
  { // bpStatus, 0x02 == success
